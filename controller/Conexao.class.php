            ...
            if(file_exists($arq)){
               ...
               switch($this->driver){
                  case "mysql":
                     ...
                     echo "<script>alertify.success('Conexão efetuada com sucesso');</script>";
                     break;
                  case "pgsql":
                     // Código do postgre
                     break;
                  case "oracle":
                     ...
                     break;
                  default:
                     die("<script>alertify.error('SGBD Incompatível com o sistema');</script>");  
               }
            }else{
               die("<script>alertify.error('Arquivo de configuração não encontrado');</script>");
            }    
         }catch(PDOException $e){
            $error = addslashes($e->getMessage());
            echo "<script>alertify.error('Erro ao conectar ao banco: {$error}');</script>";
         }catch(Exception $e){
            echo "<script>alertify.error('Erro ao processar o arquivo');</script>";
         }
      }      
   function getPDO(){
      return $this->pdo;
